...
| `CLAUDE.md` | Claude Code용 진입점 | 내용 없이 `AGENTS.md`를 읽으라고만 지시한다. |
| `GEMINI.md` | Gemini CLI용 진입점 | 위와 동일. |
...
> **이 템플릿에서는 첫 실행이 실패한다.** `CLAUDE.md`와 `GEMINI.md`에 끝 개행이 없어서 `end-of-file-fixer`가 두 파일을 고친 뒤 Failed로 끝난다. 정상 동작이다.
