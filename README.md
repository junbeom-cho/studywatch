...
닉네임 · 강의명 · 강사명 · 오늘 날짜 · 경과 시간 · 이번 달 달력 · 연속 일수를 캔버스 한 장에 그린다. 그대로 찍어서 인증에 쓴다.

화면은 셋이다 — **스톱워치**(공부하는 동안 보는 곳), **달력**(날짜별 기록 목록과 그날 찍은 스크린샷), **설정**(닉네임 · 강의 · 알림 · 배경 · 테마).

시계를 끝낼 때 누르는 버튼은 **기록**이다. "정지"라고 적으면 일시정지처럼 읽히는데, 실제로는 이번 세션을 남기고 0으로 되돌리는 되돌릴 수 없는 동작이다.

| 상태 | 기능 |
|---|---|
| 됨 | 스톱워치, 날짜, 닉네임, 강의 정보, 버리기, 스크린샷, 알림, 월 달력, 연속 일수, 기록 목록·삭제, 배경 이미지, PIP, 다크·라이트 |

PRD 의 요구사항 9개가 모두 들어갔다.
